...
class PreTrainer(object):
    ...
    def __init__(self, cfg, pwd):
        ...
        self.lr_scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, step_size=cfg.pre_step_size,
                                                            gamma=cfg.pre_gamma)
        ...
        for epoch in range(1, cfg.pre_max_epoch + 1):
            ...
            for i, batch in enumerate(tqdm_gen, 1):
                ...
                tqdm_gen.set_description(
                    'Epoch {}, Loss={:.4f} gt={:.1f} pred={:.1f} lr={:.4f}'.format(epoch, loss.item(), label_cnt,
                                                                                   pred_cnt,
            ...
            if epoch + 1 > cfg.test_start_epoch:
                ...
                if epoch % 10 == 0:
                    print('Best Epoch {}, Best Val mae={:.2f} mae={:.2f}'.format(self.record['best_model_name'],
                ...
                print('Epoch {}, Val, Loss={:.4f} mae={:.4f}  mse={:.4f}'.format(epoch, val_loss_avg, val_mae_avg,
                                                                                 val_mse_avg))
                ...
                self.record = update_model(
                    'stu',self.model.module, epoch, self.exp_path, self.exp_name, [val_mae_avg, val_mse_avg, val_loss_avg],
                    self.record,
                    self.log_txt)

                if epoch % 10 == 0:
                    print('Running Time: {}, Estimated Time: {}'.format(timer.measure(),
                                                                        timer.measure(epoch / cfg.pre_max_epoch)))
...
def vis_img(batch,name):
    ...
    for i in range(len(batch)):
        image = Image.fromarray(np.uint8(batch[i])).convert('RGB')
